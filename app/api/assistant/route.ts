...
export async function POST(req: Request) {
  ...
  const input: {
    ...
    message: string;
  } = await req.json(); // Create a thread if needed

  const threadId = input.threadId ?? (await openai.beta.threads.create({})).id; // Add a message to the thread
  ...
  return experimental_AssistantResponse(
    ...
    async ({ threadId, sendMessage, sendDataMessage }) => {
      ...
      await waitForRun(run); // Get new thread messages (after our message)
